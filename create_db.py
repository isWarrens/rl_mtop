...
if __name__ == '__main__':
    ...
    tables = [
        ...
        ("data/On-street_Car_Parking_Sensor_Data_-_2017_02_08.csv", "events", ['"ArrivalTime"', '"DepartureTime"'], []),
    ]
    ...
    for file, table, datetime_cols, time_cols in tables:
        ...
        output = Popen(["find", "/v /c "" "+ file], stdout=PIPE)

        # lines = int(Popen(["find", "/v /c "" "+ file], stdout=PIPE).communicate()[0].split(b" ")[0])
        lines = 4178
        # print('lines')
        with tqdm(total=lines) as pbar:
            with open(file, "r") as f:
                ...
                # cursor.execute(" PRAGMA table_info([roads]);")
                # col_name_list = [tuple[0] for tuple in cursor.description]

                # print("col_name_list")
                # print([tuple[0] for tuple in cursor.description])
                ...
                for data in tqdm(reader):
                    for idx in datetime_idx:
                        if len(data[idx]) == 0:
                            ...
                            data[idx] ="%d"% datetime.strptime(data[idx], "%m/%d/%Y %I:%M:%S %p").timestamp()
                            # data[idx] = datetime.strptime(data[idx], "%m/%d/%Y %I:%M:%S %p").strftime("%s")

                            # print(data[idx])
                        #用于sensor_data_2017的arrivalTime和departureTime
                    ...
                    #用于restraction的startTime和endTime
                    data = ['"' + d + '"' for d in data]

                    if len(data) == len(header):
                        # cursor.execute("INSERT INTO " + table + " (" + ", ".join(header) + ") VALUES(" + ", ".join(data) + ")")
                        cursor.execute(
                            "INSERT INTO " + table + " (" + ", ".join(header) + ") VALUES(" + ", ".join(data) + ")")
                    else:
